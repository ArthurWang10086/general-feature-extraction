...
from TimeSplit.ThreeGameSplit import ThreeGameSplit
from TimeSplit.NextDaySplit import NextDaySplit
...
if __name__ == '__main__':
    ...
    TimeSplit = NextDaySplit()
    TimeSplit2 = ThreeGameSplit()
    dirIndex = list(filter(not_None,str(args.dir).split('/')))[-1]
    resourcename = PlayerSplit.name
    with open('data/'+dirIndex+'_'+resourcename+'_featurename_old.txt.pve','w') as f2:
    ...
    for filename in filelist:
        with open(filename,'r') as f:
            ...
            endtime2 = TimeSplit2.run(items)
            endtime3 = endtime2 if endtime2<endtime else endtime
            SerialId = GetSerialId(items)
            label = 0
            if PlayerSplit.run(items) :
                for item in items:
                    if datetime.datetime.strptime(item['timestamp'], "%Y-%m-%d %H:%M:%S") <= datetime.datetime.strptime(endtime3, "%Y-%m-%d %H:%M:%S"):
    ...
    with open('data/'+dirIndex+'_'+resourcename+'_result.txt.pve','w') as f:
