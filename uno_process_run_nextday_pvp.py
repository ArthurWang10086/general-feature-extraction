...
if __name__ == '__main__':
    ...
    with open('data/'+dirIndex+'_'+resourcename+'_featurename_old.txt.pvp','w') as f2:
            ...
            if PlayerSplit.run(items) :
                for item in items:
                    if datetime.datetime.strptime(item['timestamp'], "%Y-%m-%d %H:%M:%S") <= datetime.datetime.strptime(endtime, "%Y-%m-%d %H:%M:%S")\
                            and datetime.datetime.strptime(item['timestamp'], "%Y-%m-%d %H:%M:%S") > datetime.datetime.strptime(endtime2, "%Y-%m-%d %H:%M:%S"):
    ...
    with open('data/'+dirIndex+'_'+resourcename+'_result.txt.pvp','w') as f:
